...
export default function OptimizePage() {
  ...
  useEffect(() => {
    ...
    if (cachedRaw) {
      try {
        ...
        if (!isUsableXfoilPair(xf.cl, xf.cd, xf.l_d)) {
          ...
          setOptimizationMetrics({
            ...
            maxThickness: typeof cached.final_max_thickness === "number" ? cached.final_max_thickness : 0,
          });
        ...
        if (data.meta) {
          ...
          if (!isUsableXfoilPair(xf.cl, xf.cd, xf.l_d)) {
            ...
            setOptimizationMetrics({
              ...
              xfoilLd: null,
              xfoilStatus: xf.status,
              maxThickness: (data.meta.final_max_thickness as number) ?? 0,
            });
  ...
  useEffect(() => {
    ...
    // Update best values only when XFoil converged and results are usable
    if (isUsableXfoilPair(currentMeta.cl, currentMeta.cd, currentMeta.cl_cd)) {
      setXfoilEverConverged(true);
      setBestLiftCoeff(currentMeta.cl as number);
      setBestDragCoeff(currentMeta.cd as number);
      setBestLDRatio(currentMeta.cl_cd as number);
      
      // Update ldRatioHistory only for converged points
  ...
  useEffect(() => {
    ...
    const isConverged = isUsableXfoilPair(meta.final_cl, meta.final_cd, meta.final_cl_cd);
    
    // If the final iteration converged, it's definitely ever converged
    if (isConverged) {
      setXfoilEverConverged(true);
    }

    const finalMetrics = {
      cl: isConverged ? (meta.final_cl as number) : 0,
      cd: isConverged ? (meta.final_cd as number) : 0,
      liftToDragRatio: isConverged ? (meta.final_cl_cd as number) : 0,
      loss: meta.final_loss,
      xfoilCl: isConverged ? meta.final_cl : null,
      xfoilCd: isConverged ? meta.final_cd : null,
      xfoilLd: isConverged ? meta.final_cl_cd : null,
      xfoilStatus: isConverged ? "converged" : "failed",
      maxThickness: meta.final_max_thickness ?? 0,
    };

    setOptimizationMetrics(finalMetrics);
    
    if (isConverged) {
      setBestLiftCoeff(meta.final_cl as number);
      setBestDragCoeff(meta.final_cd as number);
      setBestLDRatio(meta.final_cl_cd as number);
    }

    sessionStorage.setItem(
      "optimizationResults",
      JSON.stringify({
        xfoilCl: meta.final_cl ?? null,
        xfoilCd: meta.final_cd ?? null,
        xfoilLd: meta.final_cl_cd ?? null,
        xfoilStatus: isConverged ? "converged" : "failed",
    ...
    if (optSessionId) {
      localStorage.setItem(
        ...
        JSON.stringify({
          final_cl: meta.final_cl,
          final_cd: meta.final_cd,
          final_cl_cd: meta.final_cl_cd,
          final_loss: meta.final_loss,
          total_iterations: meta.total_iterations,
          final_max_thickness: meta.final_max_thickness,
    ...
    const finalConverged = meta.final_cl != null && meta.final_cd != null;
    if (finalConverged || xfoilEverConverged) {
      setTimeout(() => setShowResultsModal(true), 500);
    }
  }, [optComplete, completedFrame, optSessionId, xfoilEverConverged]);
              ...
              {activeSidebarTab === "results" && showResults && (
                <div className="flex-1 p-6 space-y-4 overflow-y-auto">
                  ...
                  <div className="bg-gradient-to-br from-orange-50 to-pink-50 rounded-xl border-2 border-orange-200 p-5 space-y-3">
                    ...
                    {optimizationMetrics.xfoilStatus === "converged" ? (
                      <div className="space-y-2 text-xs font-medium text-orange-800">
                        <div className="flex justify-between">
                          ...
                          <span className="font-black">
                            {formatValue(optimizationMetrics.xfoilCl)}
                          </span>
                        </div>
                        <div className="flex justify-between">
                          <span>
                          ...
                          <span className="font-black">
                            {formatValue(optimizationMetrics.xfoilCd)}
                          </span>
                        </div>
                        <div className="flex justify-between">
                          <span>Best L/D Ratio:</span>
                          <span className="font-black text-green-700">
                            {formatValue(optimizationMetrics.xfoilLd)}
                          </span>
                        </div>
                        <div className="flex justify-between">
                      ...
                      <div className="py-2 text-xs text-amber-800 bg-amber-50 border border-amber-200 rounded-lg px-3">
                        ...
                        <p>
                          The optimization reached the iteration limit but final
                          shapes failed XFoil validation.
                        </p>
                      </div>
                    )}
                  ...
                  <div className="grid grid-cols-2 gap-2 pt-3 border-t border-gray-100">
                    ...
                    <div className="bg-blue-50 p-2 rounded-lg border border-blue-100">
                      ...
                      <p className="text-sm font-bold text-gray-900 text-green-600">
                        {bestLDRatio > 0 ? bestLDRatio.toFixed(2) : "---"}
                      </p>
                    </div>
                    <div className="bg-gray-50 p-2 rounded-lg border border-gray-200">
                      ...
                      <p className="text-sm font-bold text-gray-900">
                        {bestLiftCoeff !== 0 ? formatValue(bestLiftCoeff) : "---"}
                      </p>
                    </div>
                    <div className="bg-gray-50 p-2 rounded-lg border border-gray-200">
                      <p className="text-[10px] font-black text-gray-600 uppercase tracking-tighter">
                      ...
                      <p className="text-sm font-bold text-gray-900">
                        {bestDragCoeff !== 0 ? formatValue(bestDragCoeff) : "---"}
                      </p>
                    </div>
                  </div>
                  
                  {bestLDRatio === 0 && (
                    <div className="mt-3 py-1.5 px-2 bg-amber-50 border border-amber-200 rounded text-[10px] text-amber-800 font-bold animate-pulse">
                      ⚠️ Waiting for converged XFoil results...
                    </div>
                  )}
                </div>
              )}
