...
export function downloadMetricsCSV(
  ...
  let csvContent =
    "Angle of Attack (deg),Velocity (m/s),Lift Coefficient (C_L),Drag Coefficient (C_D),Lift-to-Drag Ratio (L/D),Reynolds Number\n";

  metricsArray.forEach((m) => {
    csvContent += `${m.angleOfAttack},${m.velocity},${m.liftCoefficient.toFixed(6)},${m.dragCoefficient.toFixed(6)},${m.liftToDragRatio.toFixed(3)},${m.reynoldsNumber || "N/A"}\n`;
  });
